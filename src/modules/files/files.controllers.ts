...
export class FileController {
  constructor(
    private readonly fileService: AbstractFile,
  ...
  async uploadFile(@Req() req: any, @UploadedFile() file: Express.Multer.File) {
    ...
    try {
      ...
      const savedFile = await this.fileService.uploadFile(userId, file);
  ...
  @UseGuards(JwtAuthGuard)
  @Post()
  async getAllFiles(@Req() req: any, @Body() params:FileQueryDto) {
    let userId = req.user.user_id;
    return this.fileService.getAllFiles(userId, params);
  }

  @UseGuards(JwtAuthGuard)
  @ApiOperation({ summary: "Gets the number of existing files" })
  @Get('/fetch/size')
  async getFilesSize(@Req() req: any) {
    let userId = req.user.user_id;
    let data=await this.fileService.getFileSize(userId);
    handleResponse({status:HttpStatus.OK,message:ResponseMessages.GS,response:data});
  }
  ...
  async remove(@Req() req: any, @Param('id') id: string) {
    let userId = req.user.user_id;
    return await this.fileService.remove(userId, id);
  }
}
